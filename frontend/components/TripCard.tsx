...
export interface TripCardProps {
  ...
  days?: number;
  onClick?: () => void;
  /** Whether the current user owns this trip. Hides action buttons when false. */
  isOwner?: boolean;
  /** Called after the user confirms deletion. Caller handles the API call. */
    ...
    >
      ...
      {isOwner && (
        <div className="flex items-center gap-2 border-t border-slate-100 pt-3 mt-1">
          ...
          <Link
            href={`/trips/${id}/edit`}
            id={`trip-card-edit-${id}`}
            onClick={(e) => e.stopPropagation()}
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600 shadow-sm transition-all duration-150 hover:border-sky-300 hover:text-sky-700 hover:bg-sky-50 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2"
            aria-label={`Edit trip to ${destination}`}
          >
          ...
          {confirmDelete ? (
            <>
              ...
              <button
                type="button"
                id={`trip-card-delete-confirm-${id}`}
                onClick={(e) => {
                  e.stopPropagation();
                  onDelete?.(id);
                  setConfirmDelete(false);
                }}
                className="inline-flex items-center gap-1 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition-all duration-150 hover:bg-red-700 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2"
              >
                Yes, delete
              </button>
              <button
                ...
                className="inline-flex items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600 shadow-sm transition-all duration-150 hover:bg-slate-50 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2"
              >
                Cancel
              </button>
            </>
